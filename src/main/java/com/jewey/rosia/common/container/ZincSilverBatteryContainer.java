...
public class ZincSilverBatteryContainer extends BlockEntityContainer<ZincSilverBatteryBlockEntity> implements ButtonHandlerContainer {

    public static ZincSilverBatteryContainer create(ZincSilverBatteryBlockEntity forge, Inventory playerInventory, int windowId)
    {
        return new ZincSilverBatteryContainer(forge, windowId).init(playerInventory, 0);
    }

    private ZincSilverBatteryContainer(ZincSilverBatteryBlockEntity forge, int windowId)
    {
        super(ModContainerTypes.ZINC_SILVER_BATTERY.get(), windowId, forge);
