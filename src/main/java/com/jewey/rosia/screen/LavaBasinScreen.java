...
import com.jewey.rosia.screen.button.lava_basin.LavaBasinSlag;
...
public class LavaBasinScreen extends BlockEntityScreen<LavaBasinBlockEntity, LavaBasinContainer>
{
    ...
    protected void init() {
        super.init();
        addRenderableWidget(new LavaBasinSlag(blockEntity, getGuiLeft(), getGuiTop()));
    }
    ...
    protected void renderBg(GuiGraphics graphics, float partialTicks, int mouseX, int mouseY)
    {
        ...
        blockEntity.getCapability(Capabilities.FLUID).ifPresent(fluidHandler -> {
            ...
            FluidStack fluidStack2 = new FluidStack(ModFluids.INVAR_FLUID.source().get(), blockEntity.getSlagLevel() * 100);
            if (!fluidStack.isEmpty())
            {
                final TextureAtlasSprite sprite = RenderHelpers.getAndBindFluidSprite(fluidStack);
                final int fillHeight = (int) Math.ceil((float) 50 * fluidStack.getAmount() / (float) 1000);
                final int trashFillHeight = (int) Math.ceil((float) 50 * fluidStack2.getAmount() / (float) 1000);

                RenderHelpers.fillAreaWithSprite(graphics, sprite, leftPos + 68, topPos + 76 - fillHeight - trashFillHeight, 16, fillHeight, 16, 16);

                resetToBackgroundSprite();
            }
            if (!fluidStack2.isEmpty())
            {
                final TextureAtlasSprite sprite = RenderHelpers.getAndBindFluidSprite(fluidStack2);
                final int fillHeight = (int) Math.ceil((float) 50 * fluidStack2.getAmount() / (float) 1000);
    ...
    protected void renderTooltip(GuiGraphics graphics, int mouseX, int mouseY)
    {
        ...
        blockEntity.getCapability(Capabilities.FLUID).ifPresent(fluidHandler -> {
            FluidStack fluidStack2 = new FluidStack(ModFluids.INVAR_FLUID.source().get(), blockEntity.getSlagLevel() * 100);
            final int trashFillHeight = (int) Math.ceil((float) 50 * fluidStack2.getAmount() / (float) 1000);
            if (RenderHelpers.isInside(mouseX, mouseY, leftPos + 66, topPos + 25, 18, 50 - trashFillHeight)) {
                FluidStack fluid = fluidHandler.getFluidInTank(0);
                if (!fluid.isEmpty()) {
                    graphics.renderTooltip(font, Tooltips.fluidUnitsOf(fluid), mouseX, mouseY);
                }
            }
            if (RenderHelpers.isInside(mouseX, mouseY, leftPos + 66, topPos + 76 - trashFillHeight, 18, trashFillHeight)) {
                Component text = Component.nullToEmpty(blockEntity.getSlagLevel() * 100 + " mB of Slag");
                if (!fluidStack2.isEmpty()) {
                    graphics.renderTooltip(font, text, mouseX, mouseY);
                }
            }
